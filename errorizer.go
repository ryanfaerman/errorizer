// Most of this code (>90%) comes entirely from the stringer tool, which is
// Copyright the Go Authors. I don't pretend that this is all mine and I don't
// understand how to set the liscensing stuff properly. Please consider this a
// modification of their code as shared here: https://pkg.go.dev/golang.org/x/tools/cmd/stringer
// My modifications are also under the same BSD-3-Clause and if anyone knows
// how to attribute this in a better way, please tell me.
...
var (
	...
	output      = flag.String("output", "", "output file name; default srcdir/<type>_error.go")
...
func Usage() {
	fmt.Fprintf(os.Stderr, "Usage of errorizer:\n")
	fmt.Fprintf(os.Stderr, "\terrorizer [flags] -type T [directory]\n")
	fmt.Fprintf(os.Stderr, "\terrorizer [flags] -type T files... # Must be a single package\n")
	fmt.Fprintf(os.Stderr, "For more information, see:\n")
	fmt.Fprintf(os.Stderr, "\thttps://pkg.go.dev/github.com/ryanferman/errorizer\n")
...
func main() {
	...
	g.Printf("// Code generated by \"errorizer %s\"; DO NOT EDIT.\n", strings.Join(os.Args[1:], " "))
	g.Printf("\n")
	...
	outputName := *output
	if outputName == "" {
		baseName := fmt.Sprintf("%s_error.go", types[0])
...
func (g *Generator) generate(typeName string) {
	...
	g.Printf("\t// Re-run the errorizer command to generate them again.\n")
...
const stringOneRun = `func (i %[1]s) Error() string {
	if %[3]si >= %[1]s(len(_%[1]s_index)-1) {
		return "%[1]s(" + strconv.FormatInt(int64(i), 10) + ")"
	}
	return "%[4]s-" + strconv.FormatInt(int64(i+1), 10) + ": " + _%[1]s_name[_%[1]s_index[i]:_%[1]s_index[i+1]]
}
`
...
/*
 */
const stringOneRunWithOffset = `func (i %[1]s) Error() string {
	i -= %[2]s
	if %[4]si >= %[1]s(len(_%[1]s_index)-1) {
		return "%[1]s(" + strconv.FormatInt(int64(i + %[2]s), 10) + ")"
	}
	return "%[5]s-" + strconv.FormatInt(int64(i+1), 10) + ": " +_%[1]s_name[_%[1]s_index[i] : _%[1]s_index[i+1]]
}
`
...
func (g *Generator) buildMultipleRuns(runs [][]Value, typeName string) {
	...
	for i, values := range runs {
		if len(values) == 1 {
			g.Printf("\tcase i == %s:\n", &values[0])
			g.Printf("\t\treturn \"%s-\" + strconv.FormatInt(int64(i+1), 10) + \": \" + _%s_name_%d\n", g.pkg.name, typeName, i)
			continue
		}
		...
		g.Printf("\t\treturn \"%s-\" + strconv.FormatInt(int64(i+1), 10) + \": \" +  _%s_name_%d[_%s_index_%d[i]:_%s_index_%d[i+1]]\n",
...
const stringMap = `func (i %[1]s) Error() string {
